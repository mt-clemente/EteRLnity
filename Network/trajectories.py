...
class EpisodeBuffer:
    ...
    def clone(self):
        ...
        clone.state_buf = self.state_buf.clone()
        clone.act_buf = self.act_buf.clone()
        clone.tile_buf = self.tile_buf.clone()
        clone.tile_seq = self.tile_seq.clone()
        clone.rtg_buf = self.rtg_buf.clone()
        clone.policy_buf = self.policy_buf.clone()
        clone.mask_buf = self.mask_buf.clone()
        clone.value_buf = self.value_buf.clone()
        clone.next_value_buf = self.next_value_buf.clone()
        clone.rew_buf = self.rew_buf.clone()
        clone.final_buf = self.final_buf.clone()
        clone.adv_buf = self.adv_buf.clone()
        clone.timestep_buf = self.timestep_buf.clone()
        clone.ptr = self.ptr
